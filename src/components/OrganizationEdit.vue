...
export default {
  ...
  methods: {
          ...
          if (valid) {
            ...
            HTTP.put(url, newObj)
              .then(response => {
                ...
                this.$router.push('/organizations/edit/' + response.data.result.id)
              })
              .catch(e => {
    ...
    listContactDetails () {
      var entityId = this.$route.params.entity_id
      var url = '/organizations/' + entityId + '/contact_details/list'
      this.$router.push(url)
    },
    listLinks () {
      var entityId = this.$route.params.entity_id
      var url = '/organizations/' + entityId + '/links/list'
      this.$router.push(url)
    },
    listOtherNames () {
      var entityId = this.$route.params.entity_id
      var url = '/organizations/' + entityId + '/other_names/list'
      this.$router.push(url)
    },
    listIdentifiers () {
      var entityId = this.$route.params.entity_id
      var url = '/organizations/' + entityId + '/identifiers/list'
      this.$router.push(url)
    },
    listCitations () {
      var entityId = this.$route.params.entity_id
      var url = '/organizations/' + entityId + '/citations/list'
      this.$router.push(url)
    }
  },
  components: {
